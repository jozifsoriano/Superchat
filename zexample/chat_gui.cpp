...
void interface::sample(){
  setup();
  char str[] = "la_luna";
  mvwprintw(main,1,1,"billy: hey");
  mvwprintw(main,2,1,"mandy33: hello");
  mvwprintw(members,1,1,"billy");
  mvwprintw(members,2,1,"mandy33");
  mvwprintw(members,3,1,"la_luna");
  refresh();
  wrefresh(main);
  wrefresh(members);
  wrefresh(input);
  std::string s = get_input(input,str);
  getch();
}
